...
# coherent_info_mobius_final.py
# Coherent information vs p for d=2 Möbius/cylinder strip codes (robust GF(2) implementation).
# - Handles dependent stabilizers (row-reduction).
# - Computes m0(σ) via a right-inverse of the reduced H_Z.
# - Estimates I_c(p) ≈ k - 2 H(X_L | Σ_Z) by Monte Carlo.
# - Plots multiple sizes so curves resemble Fig. 2 in Colmenarez et al., PRResearch 6, L042014 (2024).
...
if __name__ == "__main__":
    ...
    for (L, w, l, W) in sizes:
        ...
        # -------------------------------
        # Sanity check: all destabiliser columns commute with logical Z
        # -------------------------------
        # all_commute = True
        # for i in range(code.R.shape[1]):  # loop over independent stabilizers
        #     m0_col = code.R[:, i]
        #     if np.bitwise_and(m0_col, code.l_op).sum() % 2 != 0:
        #         all_commute = False
        #         print(f"Warning: destabiliser column {i} does NOT commute with logical Z!")
        #
        # if all_commute:
        #     print("✅ All vertex destabiliser columns commute with logical Z.")
...
#     """Invert a full-rank square matrix over GF(2)."""
...
# def build_right_inverse_from_independent(H_ind: np.ndarray, pivot_cols: List[int]) -> np.ndarray:
#     """
#     Given H_ind (r x n) with full row rank r and pivot column positions,
#     build R (n x r) such that H_ind @ R = I_r (over GF(2)).
#     We form A = H_ind[:, P] (r x r) and set R[P,:] = A^{-1}, others 0.
#     """
#     r, n = H_ind.shape
#     if r == 0:
#         raise ValueError("Empty H_ind.")
#     P = np.array(pivot_cols, dtype=int)
#     A = H_ind[:, P]  # r x r
#     Ainv = gf2_invert(A)
#     R = np.zeros((n, r), dtype=np.uint8)
#     R[P, :] = Ainv
#     # Sanity: H_ind @ R == I
#     I = (H_ind @ R) % 2
#     if not np.array_equal(I, np.eye(r, dtype=np.uint8)):
#         raise RuntimeError("Failed to construct a right-inverse (H_ind @ R != I).")
#     return R



# 3,3  = 6 vertices, no twist

# -------------------------------
# Core: coherent information MC
# -------------------------------

# class CoherentInfoMC_old:
#     """
#     Estimate I_c(p) ≈ k - 2 H(X_L | Σ_Z) for CSS codes under i.i.d. X-errors with prob p.
#     Uses syndrome-consistent m0(σ): solve H_Z m0 = σ via a precomputed right-inverse
#     of the *independent* rows of H_Z.
#     """
#     def __init__(self, H_Z_full: np.ndarray, l_Z: np.ndarray, k: int = 1):
#         assert H_Z_full.dtype == np.uint8 and l_Z.dtype == np.uint8
#         assert H_Z_full.ndim == 2 and l_Z.ndim == 1 and H_Z_full.shape[1] == l_Z.shape[0]
#         self.HZ_full = H_Z_full
#         self.lZ = l_Z
#         self.n = l_Z.shape[0]
#         self.k = int(k)
#
#         # Reduce H_Z to independent rows
#         HZ_ind, row_keep, pivot_cols = gf2_row_reduce_independent(H_Z_full)
#         self.HZ_ind = HZ_ind
#         self.row_keep = np.array(row_keep, dtype=int)       # indices of kept rows in the original H_Z
#         self.pivot_cols = pivot_cols
#         self.r = HZ_ind.shape[0]
#         if self.r < H_Z_full.shape[0]:
#             print(f"⚠️ Warning: H_Z had {H_Z_full.shape[0]-self.r} dependent rows; rank = {self.r}")
#
#         # Build right-inverse for reduced system (H_ind @ R = I_r)
#         self.R = build_right_inverse_from_independent(self.HZ_ind, self.pivot_cols)  # n x r
#
#     def m0_from_sigma(self, sigma_reduced: np.ndarray) -> np.ndarray:
#         # sigma_reduced has length r (independent rows)
#         return ((self.R @ (sigma_reduced & 1)) % 2).astype(np.uint8)
#
#     def sample_trial(self, p: float, rng: np.random.Generator) -> Tuple[Tuple[int, ...], int]:
#         """
#         Sample m ~ Bernoulli(p)^n, compute full syndrome Σ_full = H_full m,
#         reduce Σ to independent components Σ_ind = Σ_full[row_keep],
#         set m0 = R Σ_ind, and return (Σ_ind, x_X) with x_X = <m - m0, l_Z> mod 2.
#         """
#         m = (rng.random(self.n) < p).astype(np.uint8)
#         sigma_full = (self.HZ_full @ m) % 2
#         sigma_ind = sigma_full[self.row_keep]
#         m0 = self.m0_from_sigma(sigma_ind)
#         diff = m ^ m0
#         x_X = int((np.bitwise_and(diff, self.lZ).sum()) % 2)
#         return tuple(int(s) for s in sigma_ind.tolist()), x_X
#
#     def estimate_H_X_given_S(self, p: float, num_samples: int, seed: int | None = None) -> float:
#         rng = np.random.default_rng(seed)
#         joint = Counter()
...
# ----------------- Suggested plotting / run parameters -----------------
# In your __main__ block, use:
#
#   code = CoherentInfoMC(H_Z, l_Z, k=1, use_destabilisers=True, dual=False, bitflip_only=True)
#
# and keep num_samples large (>= 2e5) for smoother curves.
#
# If you want to get smoother exact curves for (3,3) and (5,5), enable exact computation:
#
#   USE_EXACT_FOR_SMALL = True
#   EXACT_R_MAX = 20
#
# (You can implement an exact Walsh/Hadamard routine for small r; I can add that if you want.)
'''
class CoherentInfoMC:
    """
    Estimate I_c(p) ≈ k - 2 H(X_L | Σ) for CSS codes under i.i.d. errors.
    Now supports explicit destabiliser strings (vertex or plaquette).
    """
    def __init__(self, H_full: np.ndarray, l_op: np.ndarray, k: int = 1, use_destabilisers: bool = True, dual: bool = False):
        """
        H_full : parity-check matrix (Z-checks for X-errors or X-checks for Z-errors)
        l_op   : logical operator of the same type as the errors
        use_destabilisers : if True, build explicit destabiliser strings
        dual   : if True, assume H_full is plaquettes (X-checks), else vertices (Z-checks)
        """
        assert H_full.dtype == np.uint8 and l_op.dtype == np.uint8
        self.H_full = H_full
        self.l_op = l_op
        ...
        H_ind, row_keep, pivot_cols = gf2_row_reduce_independent(H_full)
        self.H_ind = H_ind
        self.row_keep = np.array(row_keep, dtype=int)
        self.pivot_cols = pivot_cols
        self.r = H_ind.shape[0]
        if self.r < H_full.shape[0]:
            print(f"⚠️ Warning: H_full had {H_full.shape[0]-self.r} dependent rows; rank = {self.r}")

        # Build destabiliser strings if requested
        if use_destabilisers:
            if dual:
                self.R = self.build_plaquette_destabilisers()
            else:
            ...
            # Fall back to algebraic right-inverse
            self.R = build_right_inverse_from_independent(self.H_ind, self.pivot_cols)

    def build_vertex_destabilisers(self) -> np.ndarray:
        """Explicit destabiliser strings for vertex (Z) checks, i.e., X-errors"""
        L = int(np.sqrt(self.n // 2))  # rough guess; works for square strips
        w = (self.n - L*L) // L + 1
        n = self.n
        R_vertex = np.zeros((n, self.r), dtype=np.uint8)
        for col_idx, pivot_col in enumerate(self.pivot_cols):
            if pivot_col < L * w:
                # horizontal edge: flip pivot itself
                R_vertex[pivot_col, col_idx] = 1
            else:
                # vertical edge: flip edges from pivot up to top row
                local_y = (pivot_col - L*w) // L
                local_x = (pivot_col - L*w) % L
                for y in range(local_y + 1):
                    idx = L*w + y*L + local_x
                    R_vertex[idx, col_idx] = 1
        return R_vertex

    def build_plaquette_destabilisers(self) -> np.ndarray:
        """Explicit destabiliser strings for plaquette (X) checks, i.e., Z-errors"""
        L = int(np.sqrt(self.n // 2))  # rough guess
        w = (self.n - L*L) // L + 1
        n = self.n
        R_plaquette = np.zeros((n, self.r), dtype=np.uint8)
        for col_idx, pivot_col in enumerate(self.pivot_cols):
            if pivot_col < L * w:
                R_plaquette[pivot_col, col_idx] = 1
            else:
                local_y = (pivot_col - L*w) // L
                local_x = (pivot_col - L*w) % L
                for y in range(local_y, w-1):
                    idx = L*w + y*L + local_x
                    R_plaquette[idx, col_idx] = 1
        return R_plaquette

    def m0_from_sigma(self, sigma_reduced: np.ndarray) -> np.ndarray:
        """Compute a particular solution using explicit destabilisers"""
        return ((self.R @ (sigma_reduced & 1)) % 2).astype(np.uint8)
    ...
    def sample_trial(self, p: float, rng: np.random.Generator) -> Tuple[Tuple[int, ...], int]:
        ...
        m0 = self.m0_from_sigma(sigma_ind)
        diff = m ^ m0
        # print(m0,self.H_ind,np.all((self.H_ind @ diff))==0)
        x_X = int((np.bitwise_and(diff, self.l_op).sum()) % 2)
        return tuple(int(s) for s in sigma_ind.tolist()), x_X
